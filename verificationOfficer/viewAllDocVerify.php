<?php
    ...
    session_start();
    $user = $_SESSION["verOfficerId"];

    $query = "SELECT * FROM ApprovalApplications AA INNER JOIN Documents D ON AA.DocId = D.DocId INNER JOIN metadata M ON M.DocId = D.DocId WHERE AA.status = 30 AND AA.verificationOfficerId = '$user'";
                        ...
                        <div class="documents list">
                            ...
                            <div class="document folder-documents">
                                <div class="document-content">
                                    ...
                                    <a class="text-success edit-document" href="docViewer.php?status=<?php echo $doc['DocId']?>" data-toggle=" modal" title="View"
                                            data-target="#"><i title="Approve" class="icon-eye"></i></a>
                                        <a class="text-success edit-document" href="approveVerifyDocLogic.php?status=<?php echo $doc['DocId']?>" data-toggle=" modal"
                                            data-target="#"><i title="Approve" class="icon-check"></i></a>
                                        <a class="text-danger delete-document" href="rejectVerifyDocLogic.php?status=<?php echo $doc['DocId']?>"><i title = "Reject" class="icon-ban"></i></a>
                                    </div>
                                </div>
                            </div>
